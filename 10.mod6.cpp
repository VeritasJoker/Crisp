...
int main(){
   ...
   /********** Parameters **********/
    int N, num_cat, inhib_node = 3; // number of nodes and number of final cycles
    ...
    for(int i = 0; i < num_cat; i++){
        ...
        vector<int>::iterator position = find(vect.begin(), vect.end(), inhib_node);
        if (position != vect.end()){ // if found
            vect.erase(position);
        }
        ...
        while(y != cat_cycles[i][0]){
            ...
            vect = vect_next;
            vector<int>::iterator position_two = find(vect.begin(), vect.end(), inhib_node);
            if (position_two != vect.end()){ // if found
                vect.erase(position_two);
            }
            vect_next.clear();
