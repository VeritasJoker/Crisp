...
int main(){
    ...
    long int dur = 10000000; //time steps
    double noise = 0.125; //probability of one neuron changing state
    ...
    for(int t = 0; t < dur; t++){ //duration of one run
        ...
        if(dis(gen) < noise){
            int neuron = int_dis(gen);
            vector<int>::iterator itr = find(vect_next.begin(),vect_next.end(),neuron);
            if(itr!= vect_next.end()){ //neuron exists in vector
                vect_next.erase(itr); //delete neuron
            }
            else{ //neuron does not exist in vector
                vect_next.push_back(neuron); //add neuron
            }
        }
        /*** wrap up ***/
        vect = vect_next;
